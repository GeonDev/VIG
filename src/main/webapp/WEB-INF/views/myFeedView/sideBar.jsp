		...
		<div class="row" id="sideBar">
			...
			<!-- 다른 유저의 마이 피드 방문시 해당 유저의 정보만 노출 -->
				...
				<c:when test="${user.userCode != writer.userCode}">		
					<img class="card-img-top" src="/images/uploadFiles/${writer.profileImg}" id="pImg" alt="profile_img">
			 			<div class="card-body">
					...
					<c:when test="${user.userCode == writer.userCode}">			
					 ...
					 <div class="box_body">
						 <c:if test="${sessionScope.user.role!='admin'}">
							<div class="wrap">
			 					<a id="btn_ch" class="btn btn-block" href="/myfeed/getMyFeedList?userCode=${user.userCode}">
			 						마이피드 목록</a>
			 					<br/>
			 					<a id="btn_ch" class="btn btn-block" href="/history/getMyHistoryList?userCode=${user.userCode}">
			 						내 활동 보기</a>
			 					 <br/>
			 					 <a id="btn_ch" class="btn btn-block" href="/user/updateUser?userCode=${user.userCode}">
			 						내 정보 수정</a>
			 					 <br/>	
			 				 </div>
			 			 </c:if>
			 		...
			 		<c:if test="${sessionScope.user.role=='business'}">
			 			<a id="btn_ch" class="btn btn-block" href="/chart/getChart">통 계</a>
			 				<br/>	
				 	</c:if>							
				...
				<c:if test="${sessionScope.user.role=='user'}">
				 <a class="dropdown-item" id="toggleID" href="/payment/getPaymentList" style="text-align: center;">내 결제 목록</a>
				  </c:if>
				  
				  <c:if test="${sessionScope.user.role=='business'}">
				  <a class="dropdown-item" id="toggleID" href="/payment/getPaymentList" style="text-align: center;">내 결제 목록</a>
				  <div class="dropdown-divider"></div>
				  <a class="dropdown-item" id="toggleID" href="/withdraw/getDonationList" style="text-align: center;">후원 받은 목록</a>
				   <div class="dropdown-divider"></div>
				  <a class="dropdown-item" id="toggleID" href="/withdraw/getWithdrawList" style="text-align: center;">내 출금 목록</a>
				  </c:if>
			...
			<!-- 관리자 로그인시 --> 		
			 		<c:if test="${sessionScope.user.role=='admin'}">
			 			<a type="button" class="btn btn-block" href="/user/getUserList">회원 목록</a>
			 				<br/>
			 			<a type="button" class="btn btn-block"href="/report/getReportList">신고 조회</a>
			 				<br/>
			 			<a type="button" class="btn btn-block" href="/payment/getAllPaymentList">결제 조회</a>
			 				<br/>
			 			<a type="button" class="btn btn-block" href="/withdraw/getWithdrawList">출금 조회</a>
			 				<br/>	
			 		</c:if>	
